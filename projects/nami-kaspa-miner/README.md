...
{
  // 📋 交易基本資訊
  "transaction_id": "65cf3c...",      // 交易唯一識別碼
  "hash": "880d15...",                 // 交易 hash（用於簽名）
  "mass": "1647",                      // 交易大小（影響手續費）
  "subnetwork_id": "000...000",        // 子網路 ID
  
  // 📜 Payload（可選，用於 OP_RETURN）
  "payload": "6a15...",
  
  // 📥 Inputs（花費的 UTXO）
  "inputs": [{
    "previous_outpoint_hash": "7d9588...",  // 來源交易 ID
    "previous_outpoint_index": "4",          // 來源交易的第幾個 output
    "signature_script": "41e642...",         // 🔐 簽名（證明你有私鑰）
    "sig_op_count": "1"                      // 簽名操作數
  }],
  
  // 📤 Outputs（輸出）
  "outputs": [{
    "amount": 10000000,                      // 金額（單位: sompi）
    "script_public_key": "200d77...",        // 鎖定腳本
    "script_public_key_address": "kaspatest:qq...",  // 收款地址
    "script_public_key_type": "pubkey"       // 地址類型
  }],
  
  // 🧊 區塊確認資訊（廣播後才有）
  "block_hash": ["7646da..."],               // 被打包的區塊
  "block_time": 1770123472845,               // 區塊時間
  "is_accepted": true,                       // 是否被接受
  "accepting_block_blue_score": 371935458    // DAG 確認分數
}
```

### UTXO 模型

Kaspa 使用 **UTXO (Unspent Transaction Output)** 模型：
...
**重點：**
- 每個 UTXO 只能被花費一次（全額）
- 想花一部分？創建新的 output 當「找零」
- Input 必須提供簽名，證明你有權花費
...
**Kaspa 使用 Schnorr 簽名**（比 ECDSA 更高效）
...
┌─ Input ────────────────────────────────────────────────────┐
│ From: 之前的挖礦獎勵 UTXO                                   │
│ previous_outpoint_hash: 7d958844eae63efc...                │
│ previous_outpoint_index: 4                                  │
│ signature: 41e642e01fe307c980be5f7155742ec27ee4be0c79...    │
└────────────────────────────────────────────────────────────┘
                            │
                            ▼
┌─ Outputs ──────────────────────────────────────────────────┐
...
│   to: kaspatest:qqxhwz070a3tpmz57alnc3zp67uqrw8ll7rd...    │
│   (發給自己)                                                │
│                                                             │
│ Payload (OP_RETURN):                                        │
│   6a1548656c6c6f2066726f6d204e616d692120f09f8c8a           │
│   解碼: "Hello from Nami! 🌊"                               │
└────────────────────────────────────────────────────────────┘
                            │
                            ▼
┌─ 區塊確認 ─────────────────────────────────────────────────┐
│ block_hash: 7646da6c3dd2679e90e8ecefaeda42f93bf585e621...   │
│ block_time: 2026-02-03                                      │
│ blue_score: 371,935,458                                     │
│ is_accepted: ✅ true                                        │
└────────────────────────────────────────────────────────────┘
```
